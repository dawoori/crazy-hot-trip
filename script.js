...
const cities = [
    ...
    {
        "경기도": [
            "가평군", "고양시", "과천시", "광명시", "광주시", "구리시", "군포시", "김포시", "남양주시", "동두천시", "부천시", "성남시", "수원시", "시흥시", "안산시", "안성시", "안양시", "양주시", "양평군", "여주시", "연천군", "오산시", "용인시", "의왕시", "의정부시", "이천시", "파주시", "평택시", "포천시", "하남시", "화성시", "인천광역시"
        ]
    },
    {
        "강원도": [
    ...
    {
        "충청남도": [
            "천안시", "공주시", "보령시", "아산시", "서산시", "논산시", "계룡시", "당진시", "금산군", "부여군", "서천군", "청양군", "홍성군", "예산군", "태안군", "대전광역시", "세종특별자치시"
        ]
    },
    // {
    //     "경상북도": [
    ...
    {
        "전라남도": [
            "여수시", "순천시", "목포시", "광양시", "나주시", "무안군", "해남군", "고흥군", "화순군", "영암군", "영광군", "완도군", "담양군", "장성군", "보성군", "신안군", "장흥군", "강진군", "함평군", "진도군", "곡성군", "구례군", "광주광역시"
        ]
    }
    // {
    //     "제주특별자치도": [
...
function citySelect(elem) {
    const effect = setInterval(function () {
        cityNum = Math.floor(Math.random() * cities.length);
        for (let city in cities[cityNum]) elem.innerText = city;
    }, 10);
    const effect2 = setInterval(function () {
        cityNum = Math.floor(Math.random() * cities.length);
        for (let city in cities[cityNum]) elem.innerText = city;
    }, 150);
    setTimeout(function () {
    ...
    setTimeout(function () {
        ...
        siSelect(document.querySelectorAll("#ball2")[0]);
    }, 6000);
}

function siSelect(elem) {
    let sies = cities[cityNum];
    let siNum;
    for (let aSi in sies) {
        const effect = setInterval(function () {
            siNum = Math.floor(Math.random() * sies[aSi].length);
            elem.innerText = sies[aSi][siNum];
        }, 10);
        const effect2 = setInterval(function () {
            siNum = Math.floor(Math.random() * sies[aSi].length);
            elem.innerText = sies[aSi][siNum];
...
function lottery() {
    citySelect(document.querySelectorAll("#ball1")[0]);
}
